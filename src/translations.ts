export type Language = 'zh' | 'en' | 'ja';

export const translations = {
  zh: {
    title: "GoMemo • 棋型記憶訓練",
    learningProgress: "學習進度",
    totalMastery: "總體掌握度",
    problem: "題目",
    loginSync: "登入同步",
    logout: "登出",
    remainingAttempts: "剩餘機會",
    difficulty: "難度等級",
    boardSize: "棋盤大小",
    size9: "9路",
    size13: "13路",
    size19: "19路",
    all: "全部",
    toddler: "幼幼班",
    beginner: "初學",
    basic: "基礎",
    intermediate: "中階",
    advanced: "高階",
    extreme: "極限",
    mySgf: "我的棋譜",
    uploadSgf: "上傳 SGF 檔案",
    selectMoves: "選擇記憶手數",
    confirmSgf: "確認並開始",
    sgfError: "檔案解析錯誤，請確認 SGF 格式",
    moves: "手",
    memorizing: "記憶中",
    recallMode: "作答模式",
    inProgress: "進行中",
    correct: "正解！",
    masteredPattern: "掌握棋型",
    wrong: "不對",
    tryAgain: "再試一次",
    startRecallNow: "立刻開始作答",
    blackStone: "黑子",
    whiteStone: "白子",
    eraser: "橡皮擦",
    peek: "再看一次 (10秒)",
    magnify: "放大鏡",
    exit: "退出",
    reset: "重置",
    shuffle: "隨機排序",
    startTraining: "開始作答",
    checkAnswer: "確認答案",
    nextProblem: "下一題挑戰",
    mode: "模式",
    training: "練習",
    review: "複習",
    dueForReview: "待複習",
    statsTitle: "學習進度統計",
    attempted: "已學習",
    correctCount: "已答對",
    accuracy: "答對率",
    clearStatsConfirm: "確定要清除所有學習進度嗎？",
    clearStats: "清除所有統計數據",
    login: "會員登入",
    register: "註冊帳號",
    username: "使用者名稱",
    usernamePlaceholder: "您的名稱",
    email: "電子郵件",
    password: "密碼",
    confirmPassword: "確認密碼",
    loginBtn: "登入",
    registerBtn: "註冊",
    noAccount: "還沒有帳號？立即註冊",
    hasAccount: "已有帳號？返回登入",
    passwordMismatch: "兩次輸入的密碼不一致",
    authFailed: "認證失敗",
    footer: "手筋辭典 • 經典題目 • 棋型記憶",
    noProblems: "此分類下無題目",
    noDueReviews: "目前沒有待複習的題目",
    selectProblemToStart: "點擊下方按鈕開始挑戰",
    gameOver: "遊戲結束",
    backToHome: "回到首頁",
    yourAnswer: "您的作答",
    correctSolution: "正確解答",
    confirmMoveToggle: "確認落子",
    confirmMoveBtn: "確認落子",
    cancelMoveBtn: "取消",
    folders: "資料夾",
    addFolder: "新增資料夾",
    folderName: "資料夾名稱",
    unclassified: "未分類",
    allSgfs: "所有棋譜",
    rename: "重新命名",
    delete: "刪除",
    saveTo: "儲存至",
    selectFolder: "選擇資料夾"
  },
  en: {
    title: "GoMemo • Pattern Memory",
    learningProgress: "Progress",
    totalMastery: "Mastery Score",
    problem: "Problem",
    loginSync: "Login & Sync",
    logout: "Logout",
    remainingAttempts: "Attempts",
    difficulty: "Difficulty",
    boardSize: "Board Size",
    size9: "9x9",
    size13: "13x13",
    size19: "19x19",
    all: "All",
    toddler: "Toddler",
    beginner: "Beginner",
    basic: "Basic",
    intermediate: "Intermediate",
    advanced: "Advanced",
    extreme: "Extreme",
    mySgf: "My SGF",
    uploadSgf: "Upload SGF File",
    selectMoves: "Select Moves to Memorize",
    confirmSgf: "Confirm & Start",
    sgfError: "Error parsing SGF, please check the format",
    moves: "Moves",
    memorizing: "Memorizing",
    recallMode: "Recall Mode",
    inProgress: "In Progress",
    correct: "Correct!",
    masteredPattern: "Pattern Mastered",
    wrong: "Incorrect",
    tryAgain: "Try Again",
    startRecallNow: "Recall Now",
    blackStone: "Black",
    whiteStone: "White",
    eraser: "Eraser",
    peek: "Peek (10s)",
    magnify: "Magnify",
    exit: "Exit",
    reset: "Reset",
    shuffle: "Shuffle",
    startTraining: "Start",
    checkAnswer: "Check",
    nextProblem: "Next Challenge",
    mode: "Mode",
    training: "Training",
    review: "Review",
    dueForReview: "Due",
    statsTitle: "Learning Statistics",
    attempted: "Attempted",
    correctCount: "Correct",
    accuracy: "Accuracy",
    clearStatsConfirm: "Are you sure you want to clear all progress?",
    clearStats: "Clear All Statistics",
    login: "Member Login",
    register: "Register",
    username: "Username",
    usernamePlaceholder: "Your Name",
    email: "Email",
    password: "Password",
    confirmPassword: "Confirm Password",
    loginBtn: "Login",
    registerBtn: "Register",
    noAccount: "No account? Register now",
    hasAccount: "Have an account? Back to login",
    passwordMismatch: "Passwords do not match",
    authFailed: "Authentication failed",
    footer: "Tesuji Dictionary • Classic Problems • Pattern Memory",
    noProblems: "No problems in this category",
    noDueReviews: "No reviews due at the moment",
    selectProblemToStart: "Click the button below to start",
    gameOver: "Game Over",
    backToHome: "Back to Home",
    yourAnswer: "Your Answer",
    correctSolution: "Correct Solution",
    confirmMoveToggle: "Confirm Placement",
    confirmMoveBtn: "Confirm",
    cancelMoveBtn: "Cancel",
    folders: "Folders",
    addFolder: "Add Folder",
    folderName: "Folder Name",
    unclassified: "Unclassified",
    allSgfs: "All SGFs",
    rename: "Rename",
    delete: "Delete",
    saveTo: "Save to",
    selectFolder: "Select Folder"
  },
  ja: {
    title: "GoMemo • 棋型記憶トレーニング",
    learningProgress: "学習進捗",
    totalMastery: "総合習得度",
    problem: "問題",
    loginSync: "ログイン同期",
    logout: "ログアウト",
    remainingAttempts: "残り回数",
    difficulty: "難易度",
    boardSize: "盤サイズ",
    size9: "9路盤",
    size13: "13路盤",
    size19: "19路盤",
    all: "全て",
    toddler: "幼児",
    beginner: "入門",
    basic: "初級",
    intermediate: "中級",
    advanced: "上級",
    extreme: "極限",
    mySgf: "マイ棋譜",
    uploadSgf: "SGFファイルをアップロード",
    selectMoves: "記憶する手数を選択",
    confirmSgf: "確認して開始",
    sgfError: "SGFの解析に失敗しました。形式を確認してください",
    moves: "手",
    memorizing: "記憶中",
    recallMode: "解答モード",
    inProgress: "進行中",
    correct: "正解！",
    masteredPattern: "棋型習得",
    wrong: "不正解",
    tryAgain: "もう一度",
    startRecallNow: "今すぐ解答",
    blackStone: "黒子",
    whiteStone: "白子",
    eraser: "消しゴム",
    peek: "もう一度見る (10秒)",
    magnify: "拡大鏡",
    exit: "終了",
    reset: "リセット",
    shuffle: "シャッフル",
    startTraining: "開始",
    checkAnswer: "答え合わせ",
    nextProblem: "次の問題へ",
    mode: "モード",
    training: "練習",
    review: "復習",
    dueForReview: "復習待ち",
    statsTitle: "学習統計",
    attempted: "学習済み",
    correctCount: "正解数",
    accuracy: "正解率",
    clearStatsConfirm: "全ての進捗を削除してもよろしいですか？",
    clearStats: "統計データをリセット",
    login: "ログイン",
    register: "新規登録",
    username: "ユーザー名",
    usernamePlaceholder: "お名前",
    email: "メールアドレス",
    password: "パスワード",
    confirmPassword: "パスワード（確認）",
    loginBtn: "ログイン",
    registerBtn: "登録",
    noAccount: "アカウントをお持ちでない方はこちら",
    hasAccount: "アカウントをお持ちの方はこちら",
    passwordMismatch: "パスワードが一致しません",
    authFailed: "認証に失敗しました",
    footer: "手筋辞典 • 古典問題 • 棋型記憶",
    noProblems: "このカテゴリーには問題がありません",
    noDueReviews: "現在、復習待ちの問題はありません",
    selectProblemToStart: "下のボタンを押して開始",
    gameOver: "ゲームオーバー",
    backToHome: "ホームに戻る",
    yourAnswer: "あなたの解答",
    correctSolution: "正解",
    confirmMoveToggle: "着手確認",
    confirmMoveBtn: "着手を確定",
    cancelMoveBtn: "キャンセル",
    folders: "フォルダ",
    addFolder: "フォルダを追加",
    folderName: "フォルダ名",
    unclassified: "未分類",
    allSgfs: "すべての棋譜",
    rename: "名前を変更",
    delete: "削除",
    saveTo: "保存先",
    selectFolder: "フォルダを選択"
  }
};
